<x-layout>
    <div class="max-w-6xl mx-auto p-6 grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div>
            ...
            <form action="{{ route('checkout.shipping.store') }}" method="POST" class="space-y-6">
                @csrf
                <div class="space-y-4">
                    @php
                        $shippingOptions = [
                    ...
                    @foreach($shippingOptions as $key => $option)
                        <label class="block border rounded-lg p-4 cursor-pointer hover:border-[#501A2E]">
                            <div class="flex items-start gap-4">
                                <input type="radio" name="shipping_method" value="{{ $key }}" class="mt-1 accent-[#501A2E] w-5 h-5" required>
                                <div>
        ...
        <div class="bg-white rounded-xl p-6 shadow">
            ...
            @foreach($cartItems as $item)
                <x-cart.cart-item :item="$item">
                    <x-slot:image>
                        <img src="{{ asset('assets/product/' . $item->product->img_url) }}" class="w-24 h-24 object-cover rounded" />
                    </x-slot:image>
                    <x-slot:details>
